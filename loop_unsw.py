...
def Run_nbrcomp(components,datapts=0,gmm='N'):
    print(components)
    print('unsw-nb15')
...
#######################################################################################
def EP_hypers_cen(Normalized_training_df, components,meth,mloops=8,fac=10):
    ...
    def tilted_invgam(site_data, cav_nat_params, method='lhood'):
        ...
        if method == 'lhood':
            rvfl_f = BRVFL_cl_f(neurons=middle, random_state=2, sigma=params[1] / (1 + params[0]),
                                Mean=np.zeros((middle, inp)), Gamma=np.eye(middle))
        else:
            rvfl_f = BRVFL_cl_f(neurons=middle, random_state=2, Mean=np.zeros((middle, inp)),
                                Gamma=(params[1] / (1 + params[0])) * np.eye(middle))
        rvfl_f.fit(site_data, site_data)  # to get the mean W_0
        rvfl_f.fit_hp(site_data, site_data)  # to get tot_sq_res
        if method == 'lhood':
            new_alpha = params[0] + rvfl_f.nbr_data / 2  # gelman 3rd edition bayesian data analysis pg 43 ..scaled inv chi^2(a,b) = IG(a/2,ab/2)
            new_beta = params[1] + rvfl_f.tot_sq_res / 2
        else:
            new_alpha = params[0] + middle / 2
            new_beta = params[1] + np.sum(np.sum((rvfl_f.W_o) ** 2, axis=0)) / 2  # np.mean(np.sum((rvfl_f.W_o) ** 2, axis=0))

        tilted_nat_params = invert_invgamma_params(new_alpha, new_beta, method='to_natural')

        return tilted_nat_params

    def delta_x_change(deltas, all_change_params):
        # Sum over sites,, (sites_delta,sites_changes_params)

        delxchanges = initialise_nat_site_igamma()
        for site, v in all_change_params.items():
            for k1, v1 in all_change_params[site].items():
                delxchanges[k1] += min(deltas) * all_change_params[site][k1]

        return delxchanges

    def SiteUpdate(sites_delta, site):
        # operation at site. receive delta from central
        sites_nat_params[site] = update_site(sites_changes_params[site], sites_nat_params[site], delta=min(sites_delta))
        return sites_nat_params[site]

    def Site_Operation(global_nat_params, sites_nat_params, site, method='lhood'):
        # operation at site. compute cavity, tilted and changes. receive global nat params from central
        sites_cavity_params = cavity_at_site(global_nat_params, sites_nat_params[site])
        sites_tilted_params = tilted_invgam(sites_data[site], sites_cavity_params, method=method)
    ...
    def Central_Operation(sites_changes_params):
        ...
        sites_delta = 1 + np.zeros(components)
        central_sites_nat_params = {}
        for site, v in sites_changes_params.items():
            print('site: {}'.format(site))
            site_cavity = initialise_nat_site_igamma()
            while (site_cavity['alpha'] >= -1) or (site_cavity['beta'] >= 0):
                print('not right!!')
                sites_delta[site - 1] = 0.5 * sites_delta[
                    site - 1]  # currently using only one value of delta (delta=1 works!)
                ## causes global params to be smaller which may result in negative definite cavity for the previous already checked sites
                # to have a different delta for each site is a multi objective optimisation problem. Added computation puts cavity closer to iteration results
                # for each site and strengthens the cavity prior which may not be good.
                # have to change delta for all sites together because reducing for later sites in the for loop. take the smallest delta that works.
                central_sites_nat_params[site] = update_site(sites_changes_params[site], sites_nat_params[site],
                                                             delta=min(sites_delta))
                new_global_params = update_site(delta_x_change(sites_delta, sites_changes_params), global_nat_params)
                # this line above is outside the for loop. d.n depend on site. But the global params will be affected for each site as delta is changed.
                # but it is recomputed in each loop for simplicity.
                # the function update_site is reused here to update the global params.
                site_cavity = cavity_at_site(new_global_params, central_sites_nat_params[site])
        print(min(sites_delta))
        return new_global_params, sites_delta
    ...
    if meth == 'lhood':
        global_nat_params = initialise_nat_site_igamma(-100, -1)  # (-1-10**-5,-10**-5)  #(-100, -1) -> initial value of sigma^2 =  0.01
    else:
        global_nat_params = initialise_nat_site_igamma(-10, -1000)  # (-1, -1)->initial value of gamma-1 =1, (-10, -1000) for 100.  #(-1-10**-5,-10**-5)
    ...
    while loop <= max_loop:
        ...
        site_results = Parallel(n_jobs=4)(
            delayed(SiteUpdate)(sites_delta, site) for site in list(range(1, components + 1)))  # chagne here
        ...
        for site in list(range(1, components + 1)):  # nbr of components = nbr of sites
            print('site: {}'.format(site))
            sites_changes_params[site] = Site_Operation(global_nat_params, sites_nat_params, site, method=meth)
        ...
        global_nat_params, sites_delta = Central_Operation(sites_changes_params)
        loop += 1
    ...
    ##################################################################################################
    # using global params
    shape, rate = invert_invgamma_params(global_nat_params['alpha'], global_nat_params['beta'], method='to_shape,rate')
...
##################################################################################################
def EP_w_cen(Normalized_training_df,Normalized_testing_df, labels, components,sig,gam,fac=10,delt=1,mloops=10,testing_d=True):
    ...
    # sig = 0.00019219715558492555 #0.00016439571713149488, 0.00019219715558492555 -crac central,84 (10*inp)
    # gams = 0.005337585964565251 #0.3704632980796816, 0.005337585964565251 crac-central,84 (10*inp)
    ...
    global_nat_params = initialise_nat_site(middle, inp, init_val=1 / gams)  # 1.0/100  #init_val=1/0.019501351863718694
    ...
    def tilted_brvfl(site_data, cav_nat_params, method='closed'):
        """
        At each site perform bayesian inference and compute tilted distribution
        Not using eta right now
        """
        ...
        if method == 'closed':  # this is closed form posterior includes the normalizing term
            rvfl_f = BRVFL_cl_f(neurons=middle, random_state=2, sigma=sig, Mean=params[0], Gamma=params[1])
    ...
    def Site_Operation(global_nat_params, sites_nat_params, site, method='closed'):
        # operation at site. compute cavity, tilted and changes. receive global nat params from central
        sites_cavity_params = cavity_at_site(global_nat_params, sites_nat_params[site])
        sites_tilted_params = tilted_brvfl(sites_data[site], sites_cavity_params, method=method)
        sites_changes_params = change_in_site(sites_nat_params[site], sites_cavity_params, sites_tilted_params)
        return sites_changes_params

    def Central_Operation(sites_changes_params,delta=delt):
        # receive site changes from sites. outputs global params and delta
        # delta needs to be reduced if site cavity dist are not pos definite
        print('central operation')
        sites_delta = 2*delta + np.zeros(components)
        central_sites_nat_params = {}
        for site, v in sites_changes_params.items():
            print('site: {}'.format(site))
            ...
            while not is_pd(site_cavity['Q']):
                ...
                sites_delta[site - 1] = 0.5 * sites_delta[ site - 1]  # currently using only one value of delta (delta=1 works!)
                ## causes global params to be smaller which may result in negative definite cavity for the previous already checked sites
                # to have a different delta for each site is a multi objective optimisation problem. Added computation puts cavity closer to iteration results
                # for each site and strengthens the cavity prior which may not be good.
                # have to change delta for all sites together because reducing for later sites in the for loop. take the smallest delta that works.
    ...
    # meth = 'vi'  # change here
    # meth = 'vi_advi' #diagonal covariance
    # meth = 'mcmc'
    # meth = 'mcmc_diag'
    ...
    #mean_site_changes_r, mean_site_changes_Q = avchange_site_per_iter(changes, max_loop, components)
    #global_changes_r, global_changes_Q = change_global(global_params_iters, global_nat_params, max_loop+1)
    print('done')
    return global_nat_params,changes,global_params_iters,test_auc
    #return (scores,scores2)
    #return (test_auc,test_auc2)
    #return (map_est, var_est)

#pd.DataFrame(global_nat_params['r']).to_csv('global_mean.csv')  #redo with correct gams and sigs
#pd.DataFrame(global_nat_params['Q']).to_csv('global_cov.csv')
...
##################################################################################################
def EP_w_cen_onebyone(Normalized_training_df,Normalized_testing_df, labels, components,sig,gam,fac=10,delt=1,mloops=10,testing_d=True):
    #adds site changes one by one to central... ensure maxloop == number of sites
    ...
    def delta_x_change(deltas, all_change_params,prev):
        """ Sum over sites,, (sites_delta,sites_changes_params)
        """
        delxchanges = initialise_nat_site(middle, inp)
        for site in prev:
    ...
    def SiteUpdate(sites_delta, site):
        # operation at site. receive delta from central
        sites_nat_params[site] = update_site(sites_changes_params[site], sites_nat_params[site], delta=sites_delta)
        return sites_nat_params[site]
    ...
    def Central_Operation(sites_changes_params,site,delta=delt):
        # receive site changes from sites. outputs global params and delta
        # delta needs to be reduced if site cavity dist are not pos definite
        print('central operation')
        sites_delta = 2*delta
        ...
        while not is_pd(site_cavity['Q']):
            print('not positive definite!')
            sites_delta = 0.5 * sites_delta  # currently using only one value of delta (delta=1 works!)
            ## causes global params to be smaller which may result in negative definite cavity for the previous already checked sites
            # to have a different delta for each site is a multi objective optimisation problem. Added computation puts cavity closer to iteration results
            # for each site and strengthens the cavity prior which may not be good.
            # have to change delta for all sites together because reducing for later sites in the for loop. take the smallest delta that works.
            central_sites_nat_params = update_site(sites_changes_params[site], sites_nat_params[site],sites_delta)
            new_global_params = update_site(delta_x_change(sites_delta, sites_changes_params,prev), global_nat_params)
            # this line above is outside the for loop. d.n depend on site. But the global params will be affected for each site as delta is changed.
            # but it is recomputed in each loop for simplicity.
            # the function update_site is reused here to update the global params.
            site_cavity = cavity_at_site(new_global_params, central_sites_nat_params)
        print(sites_delta)
        return new_global_params, sites_delta
    ...
    print('all sites complete')
    changes[loop] = sites_changes_params.copy()  # for analysis later
    global_nat_params, sites_delta = Central_Operation(sites_changes_params,loop+1)
    global_params_iters[loop] = global_nat_params.copy()
    loop+=1
    while loop<max_loop:
        sites_nat_params[1] = SiteUpdate(sites_delta, 1)
        sites_changes_params[1] = Site_Operation(global_nat_params, sites_nat_params, 1, method=meth)
        global_nat_params, sites_delta = Central_Operation(sites_changes_params, loop+1)
        global_params_iters[loop] = global_nat_params.copy()
        loop += 1
